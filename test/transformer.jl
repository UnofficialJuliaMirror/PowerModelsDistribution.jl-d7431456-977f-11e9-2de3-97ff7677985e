...
@testset "test transformer pf - acp" begin
    @testset "two winding transformer pf" begin
        @testset "2w transformer pf yy" begin
        ...
        @testset "2w transformer pf dy_lead" begin
        ...
        @testset "2w transformer pf dy_lag" begin
        ...
        @testset "2w transformer opf_oltc yy" begin
        ...
        @testset "2w transformer pf yy - banked transformers" begin
    ...
    @testset "three winding transformer pf" begin
        @testset "3w transformer pf dyy - all non-zero"  begin
        ...
        @testset "3w transformer pf dyy - some non-zero" begin
        ...
        @testset "3w transformer pf dyy - all zero" begin
        ...
        @testset "3w transformer pf dyy - %loadloss=0" begin
        ...
        @testset "3w transformer pf dyy - voltage base" begin
...
@testset "test transformer pf - acr" begin
    @testset "two winding transformer pf" begin
        @testset "2w transformer pf yy" begin
            file = "../test/data/opendss/ut_trans_2w_yy.dss"
            pmd_data = PMD.parse_file(file)
            sol = PMD.run_mc_pf(pmd_data, PMs.ACRPowerModel, ipopt_solver, multiconductor=true)
            id = bus_name2id(pmd_data, "3")
            @test norm(sol["solution"]["bus"]["$id"]["vm"]-[0.87451, 0.8613, 0.85348], Inf) <= 1.5E-5
            @test norm(sol["solution"]["bus"]["$id"]["va"]*180/π-[-0.1, -120.4, 119.8], Inf) <= 0.5E-2
        end
        ...
        @testset "2w transformer pf dy_lead" begin
            ...
            @test norm(sol["solution"]["bus"]["$id"]["vm"]-[0.87391, 0.86055, 0.85486], Inf) <= 1.5E-5
            @test norm(sol["solution"]["bus"]["$id"]["va"]*180/π-[29.8, -90.4, 149.8], Inf) <= 0.5E-2
        end
        ...
        @testset "2w transformer pf dy_lag" begin
            ...
            @test norm(sol["solution"]["bus"]["$id"]["vm"]-[0.92092, 0.91012, 0.90059], Inf) <= 1.5E-5
            @test norm(sol["solution"]["bus"]["$id"]["va"]*180/π-[-30.0, -150.4, 89.8], Inf) <= 0.5E-2
        end
        ...
        @testset "2w transformer opf_oltc yy" begin
            ...
            pmd_data["transformer"]["1"]["fixed"] = PMs.MultiConductorVector(zeros(Bool, 3))
            pmd_data["transformer"]["2"]["fixed"] = PMs.MultiConductorVector(zeros(Bool, 3))
            pm = PMs.build_model(pmd_data, PMs.ACRPowerModel, PMD.post_mc_opf_oltc, ref_extensions=[PMD.ref_add_arcs_trans!], multiconductor=true)
            sol = PMs.optimize_model!(pm, ipopt_solver)
            # check that taps are set as to boost the voltage in the branches as much as possible;
            # this is trivially optimal if the voltage bounds are not binding
            # and without significant shunts (both branch and transformer)
            ...
            @test norm(sol["solution"]["bus"]["$id"]["vm"]-[1.0352, 1.022, 1.0142], Inf) <= 1E-4
            @test norm(sol["solution"]["bus"]["$id"]["va"]*180/π-[-0.1, -120.4, 119.8], Inf) <= 0.5E-2
        end
        ...
        @testset "2w transformer pf yy - banked transformers" begin
            ...
            pmd1 = PMD.parse_file(file)
            pmd2 = PMD.parse_file(file; bank_transformers=false)
            result1 = PMD.run_mc_pf(pmd1, PMs.ACRPowerModel, ipopt_solver)
            result2 = PMD.run_mc_pf(pmd2, PMs.ACRPowerModel, ipopt_solver)
    ...
    @testset "three winding transformer pf" begin
        @testset "3w transformer pf dyy - all non-zero"  begin
            ...
            @test norm(sol["solution"]["bus"]["$id"]["vm"]-[0.9318, 0.88828, 0.88581], Inf) <= 1.5E-5
            @test norm(sol["solution"]["bus"]["$id"]["va"]*180/π-[30.1, -90.7, 151.2], Inf) <= 0.5E-2
        end
        ...
        @testset "3w transformer pf dyy - some non-zero" begin
            ...
            @test norm(sol["solution"]["bus"]["$id"]["vm"]-[0.93876, 0.90227, 0.90454], Inf) <= 1.5E-5
            @test norm(sol["solution"]["bus"]["$id"]["va"]*180/π-[31.6, -88.8, 153.3], Inf) <= 0.5E-2
        end
        ...
        @testset "3w transformer pf dyy - all zero" begin
            ...
            @test norm(sol["solution"]["bus"]["$id"]["vm"]-[0.97047, 0.93949, 0.946], Inf) <= 1.5E-5
            @test norm(sol["solution"]["bus"]["$id"]["va"]*180/π-[30.6, -90.0, 151.9], Inf) <= 0.5E-2
        end
        ...
        @testset "3w transformer pf dyy - %loadloss=0" begin
            ...
            @test norm(sol["solution"]["bus"]["$id"]["vm"]-[0.969531, 0.938369, 0.944748], Inf) <= 1.5E-5
            @test norm(sol["solution"]["bus"]["$id"]["va"]*180/π-[30.7, -90.0, 152.0], Inf) <= 0.5E-2
        end
        ...
        @testset "3w transformer pf dyy - voltage base" begin
            ...
            PMD._adjust_base!(pmd2, start_at_first_tr_prim=false)
            scale_2to1 = pmd2["bus"]["3"]["base_kv"]/pmd1["bus"]["3"]["base_kv"]
            sol1 = PMD.run_mc_pf(pmd1, PMs.ACRPowerModel, ipopt_solver, multiconductor=true)
            sol2 = PMD.run_mc_pf(pmd2, PMs.ACRPowerModel, ipopt_solver, multiconductor=true)
            for bus_id_str in keys(sol1["solution"]["bus"])
                vm1 = sol1["solution"]["bus"][bus_id_str]["vm"]
                vm2 = sol2["solution"]["bus"][bus_id_str]["vm"]
                va1 = PMD._wrap_to_pi(sol1["solution"]["bus"][bus_id_str]["va"][:])
